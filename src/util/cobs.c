...
void COBSEncodeAddData(COBSState* pState, const void* const pData, uint32_t dataSize)
{
    for(uint32_t i = 0; i < dataSize; i++)
        COBSEncodeAddByte(pState, ((const uint8_t* const)pData)[i]);
}
...
int16_t COBSEncode(const uint8_t* pIn, uint32_t sizeIn, uint8_t* pOut, uint32_t sizeOut, uint32_t* pBytesWritten)
{
    ...
    int16_t result = COBSEncodeStart(&state, sizeIn, pOut, sizeOut);
    if(result)
        return result;

    COBSEncodeAddData(&state, pIn, sizeIn);

    COBSEncodeFinalize(&state, pBytesWritten);
